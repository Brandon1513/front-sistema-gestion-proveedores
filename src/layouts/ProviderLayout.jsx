...
export const ProviderLayout = () => {
  ...
  // ✅ URL correcta: /provider/documents/expiring
            ...
            {expiringDocs.map(doc => (
              <div key={doc.id}
                ...
                <div className="flex-1 min-w-0">
                  {/* ✅ Campos correctos según la estructura del backend */}
    ...
    <div className="flex h-screen bg-gradient-to-br from-gray-50 to-gray-100">
      ...
      <aside className={`hidden md:flex md:flex-shrink-0 transition-all duration-300 ${sidebarCollapsed ? 'md:w-16' : 'md:w-72'}`}>
        <div className={`flex flex-col bg-white border-r-2 border-gray-200 shadow-lg transition-all duration-300 ${sidebarCollapsed ? 'w-16' : 'w-72'}`}>
          {/* ✅ Header con botón colapsar centrado */}
      ...
      <div className="flex flex-col flex-1 overflow-hidden">
        ...
        <header className="sticky top-0 z-10 items-center justify-end hidden gap-3 px-8 bg-white border-b border-gray-100 shadow-sm md:flex h-14">
          <BellButton/>
          {/* Usuario en la barra de notificación
          <div className="flex items-center gap-2 px-3 py-1.5 rounded-xl bg-gray-50 border border-gray-200">
            <div className="flex items-center justify-center flex-shrink-0 w-6 h-6 rounded-full bg-gradient-primary">
              <User className="w-3.5 h-3.5 text-white"/>
            </div>
            <span className="text-sm font-medium text-gray-700 truncate max-w-[160px]">{user?.name || 'Proveedor'}</span>
          </div>*/}
        </header>
