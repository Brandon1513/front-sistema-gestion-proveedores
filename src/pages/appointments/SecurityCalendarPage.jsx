...
import {
  ...
  History, Search, Filter, FlaskConical, Ban,
} from 'lucide-react';
    ...
    <div className="w-full max-w-sm bg-white shadow-2xl rounded-2xl">
      <div className="flex items-center justify-between px-6 py-4 border-b border-gray-200 bg-gradient-to-r from-orange-50 to-red-50 rounded-t-2xl">
        <div className="flex items-center gap-3">
          <div className="flex items-center justify-center w-10 h-10 bg-orange-500 rounded-lg shadow-md">
      ...
      <div className="p-6 space-y-4">
        <div className="p-4 space-y-2 border-2 border-orange-200 rounded-xl bg-orange-50">
        ...
        {appointment.no_show_notes ? (
          <div className="p-3 border border-gray-200 rounded-xl bg-gray-50">
            <p className="mb-1 text-xs font-semibold text-gray-500">Observaciones registradas:</p>
            <p className="text-sm italic text-gray-700">"{appointment.no_show_notes}"</p>
          </div>
        ) : (
          <p className="text-xs text-center text-gray-400">Sin observaciones adicionales</p>
        )}
  ...
  <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/50 backdrop-blur-sm">
    <div className="w-full max-w-sm bg-white shadow-2xl rounded-2xl">
      <div className="flex items-center justify-between px-6 py-4 border-b border-gray-200 bg-gradient-to-r from-amber-50 to-orange-50 rounded-t-2xl">
        <div className="flex items-center gap-3">
          <div className="flex items-center justify-center w-10 h-10 rounded-lg shadow-md bg-amber-500">
      ...
      <div className="p-6 space-y-3">
        <p className="mb-2 text-sm text-gray-600">Esta cita está pendiente de registro. ¿El proveedor llegó o no se presentó?</p>
        <button onClick={onConfirm}
          className="flex items-center w-full gap-3 p-4 text-left transition-all border-2 border-green-200 rounded-xl bg-green-50 hover:bg-green-100 hover:border-green-300">
          <div className="flex items-center justify-center flex-shrink-0 w-10 h-10 bg-green-100 rounded-xl">
        ...
        <button onClick={onNoShow}
          className="flex items-center w-full gap-3 p-4 text-left transition-all border-2 border-orange-200 rounded-xl bg-orange-50 hover:bg-orange-100 hover:border-orange-300">
          <div className="flex items-center justify-center flex-shrink-0 w-10 h-10 bg-orange-100 rounded-xl">
...
const DayAppointmentsModal = ({ dateStr, appointments, blocks = [], mode, onSelect, onClose }) => {
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/50 backdrop-blur-sm">
      <div className="bg-white rounded-2xl shadow-2xl w-full max-w-md max-h-[80vh] overflow-y-auto">
        ...
        <div className="p-4 space-y-2">
          ...
          <p className="mb-3 text-xs text-gray-500">Selecciona la {isAccess ? 'visita' : 'entrega'} que deseas ver:</p>
          {appointments.map(a => (
            <button key={a.id} type="button" onClick={() => onSelect(a)}
              className="flex items-center w-full gap-3 p-3 text-left transition-all border-2 border-gray-200 rounded-xl hover:border-gray-300 hover:bg-gray-50/50 group">
              ...
              <ChevronRight className="flex-shrink-0 w-4 h-4 text-gray-300 group-hover:text-gray-500"/>
            </button>
          ))}
        </div>
      </div>
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/50 backdrop-blur-sm">
      <div className="w-full max-w-sm bg-white shadow-2xl rounded-2xl">
        <div className="flex items-center justify-between px-6 py-4 border-b border-gray-200 bg-gradient-to-r from-orange-50 to-red-50 rounded-t-2xl">
          <div className="flex items-center gap-3">
            <div className="flex items-center justify-center w-10 h-10 bg-orange-500 rounded-lg shadow-md"><UserX className="w-5 h-5 text-white"/></div>
        ...
        <div className="p-6 space-y-4">
          <div className="p-3 border border-orange-200 rounded-xl bg-orange-50">
            ...
            <p className="mt-1 text-xs text-orange-600">La cita quedará con estado "No se presentó". Compras podrá gestionarla después.</p>
          </div>
          <div>
          ...
          <div className="flex gap-3 pt-2">
            <Button onClick={() => mutation.mutate()} loading={mutation.isPending} leftIcon={<UserX className="w-4 h-4"/>}
              className="flex-1 font-semibold text-white bg-gradient-to-r from-orange-500 to-red-500 hover:from-orange-600 hover:to-red-600 rounded-xl">
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/50 backdrop-blur-sm">
      <div className="bg-white rounded-2xl shadow-2xl w-full max-w-md max-h-[90vh] overflow-y-auto">
        <div className="flex items-center justify-between px-6 py-4 border-b border-gray-200 bg-gradient-to-r from-amber-50 to-orange-50 rounded-t-2xl">
          <div className="flex items-center gap-3">
            <div className="flex items-center justify-center w-10 h-10 rounded-lg shadow-md bg-amber-500"><Shield className="w-5 h-5 text-white"/></div>
        ...
        <div className="p-6 space-y-5">
          <div className={`p-3 rounded-xl border-2 ${typeCfg.border} ${typeCfg.bg} space-y-1.5`}>
            <div className="flex items-center gap-2"><span className={`w-2.5 h-2.5 rounded-full ${typeCfg.dot}`}/><p className="text-sm font-bold text-gray-900">{appointment.provider?.business_name}</p></div>
            <p className="text-xs text-gray-500">{typeCfg.label} · Cita a las {appointment.appointment_time} hrs</p>
            {appointment.vehicle_display && <p className="flex items-center gap-1 text-xs text-gray-600"><Truck className="w-3 h-3"/>{appointment.vehicle_display}</p>}
            {appointment.driver_display  && <p className="flex items-center gap-1 text-xs text-gray-600"><User className="w-3 h-3"/>{appointment.driver_display}</p>}
          </div>
          <div>
              ...
              <div className="space-y-2">
                {(docsConfig?.docs||[]).map(doc => (
                  <label key={doc.key} className={`flex items-center gap-3 p-3 rounded-xl border-2 cursor-pointer transition-all ${physicalDocs[doc.key]?'border-green-300 bg-green-50':doc.required?'border-red-300 bg-red-50':'border-gray-200 bg-gray-50'}`}>
                    <input type="checkbox" checked={!!physicalDocs[doc.key]} onChange={() => toggleDoc(doc.key)} className="w-4 h-4 text-green-600 rounded"/>
                    ...
                    {physicalDocs[doc.key]?<CheckCircle className="flex-shrink-0 w-4 h-4 text-green-500"/>:doc.required?<XCircle className="flex-shrink-0 w-4 h-4 text-red-400"/>:null}
                  </label>
                ))}
              </div>
              {missingRequired.length > 0 && (
                <div className="flex items-start gap-2 p-3 mt-2 border border-red-200 bg-red-50 rounded-xl">
      ...
      <div className="flex items-center justify-between p-4">
        <div className="flex items-center flex-1 min-w-0 gap-4">
          ...
          <div className="flex-1 min-w-0">
            ...
            ) : !isNoShow && <p className="flex items-center gap-1 mt-1 text-xs text-amber-600"><AlertCircle className="w-3 h-3"/>Proveedor aún no completó vehículo/chofer</p>}
            {isNoShow && appointment.no_show_notes && <p className="mt-1 text-xs italic text-orange-600">"{appointment.no_show_notes}"</p>}
        ...
        <div className="flex flex-col flex-shrink-0 gap-2 ml-4">
          {canAct && (
            <>
              <Button size="sm" onClick={() => onConfirm(appointment)} leftIcon={<Shield className="w-3.5 h-3.5"/>}
                className="font-semibold text-white bg-gradient-to-r from-amber-500 to-orange-500 hover:from-amber-600 hover:to-orange-600 whitespace-nowrap">Confirmar</Button>
        ...
        <div className="flex items-center flex-1 min-w-0 gap-4">
          ...
          <div className="flex-1 min-w-0">
            <div className="flex flex-wrap items-center gap-2 mb-1">
              <p className="font-bold text-gray-900 truncate">{appointment.provider?.business_name}</p>
              {isNoShow && <span className="inline-flex items-center gap-1 px-2 py-0.5 rounded-full text-xs font-semibold bg-orange-100 text-orange-700 border border-orange-200"><UserX className="w-3 h-3"/>No se presentó</span>}
              {!isNoShow && !appointment.is_entry_confirmed && <span className="inline-flex items-center gap-1 px-2 py-0.5 rounded-full text-xs font-semibold bg-amber-100 text-amber-700 border border-amber-200"><Shield className="w-3 h-3"/>Espera Seguridad</span>}
            ...
            {isReviewed && <p className="mt-1 text-xs text-gray-400">Revisado por {appointment.reception_reviewed_by} · {appointment.reception_reviewed_at}</p>}
            {isNoShow && appointment.no_show_notes && <p className="mt-1 text-xs italic text-orange-600">"{appointment.no_show_notes}"</p>}
          </div>
        </div>
        {isReviewed && (
          <button onClick={() => setExpanded(!expanded)} className="flex-shrink-0 p-2 ml-4 text-gray-400 rounded-lg hover:bg-gray-100">
...
export const SecurityCalendarPage = () => {
  ...
  // ✅ Bloques de calendario (solo lectura para seguridad — se cargan para todo el mes visible)
  ...
  // ✅ Bloques por fecha (compartidos entre ambas pestañas)
    ...
    <div className="overflow-hidden bg-white border-2 border-gray-200 rounded-xl">
      ...
      <div className="grid grid-cols-7">
        {calCells.map((day, idx) => {
          ...
          const dots = isAccess ? [
            dayAppts.some(a => a.has_missing_docs)  && <span key="miss" className="w-2 h-2 bg-red-500 rounded-full"/>,
            dayAppts.some(a => a.status==='no_show') && <span key="ns"   className="w-2 h-2 bg-orange-400 rounded-full"/>,
          ] : [
            dayAppts.some(a => a.reception_status==='rejected'||a.reception_status==='partial') && <span key="rej"  className="w-2 h-2 bg-red-400 rounded-full"/>,
            dayAppts.some(a => a.status==='no_show')                                             && <span key="ns"   className="w-2 h-2 bg-orange-400 rounded-full"/>,
  ...
  const renderWeek = (renderCard) => (
    isLoading
      ? <div className="flex items-center justify-center h-48"><div className="w-10 h-10 border-4 rounded-full border-t-amber-500 animate-spin"/></div>
      : (
            ...
            return (
              <div key={dateStr} className={`bg-white border-2 rounded-xl ${isToday?'border-amber-300':'border-gray-200'}`}>
                ...
                <div className="p-3 space-y-2">
                  {/* ✅ Bloqueos del día, antes de las citas */}
                  {dayBlocks.map(b => <BlockCard key={`b-${b.id}`} block={b}/>)}
                  {dayAppts.length>0
                    ? dayAppts.map(a => renderCard(a))
                    : (dayBlocks.length===0 && <p className="py-4 text-sm text-center text-gray-400">Sin visitas</p>)}
                </div>
              </div>
            );
          })}
    ...
    <div className="space-y-6">
        ...
        <div className="flex flex-wrap items-center justify-between gap-3">
          ...
          {isAccess && (
            <button onClick={() => refetchAccess()}
              className="flex items-center gap-2 px-4 py-2 text-sm font-semibold border text-amber-700 bg-amber-100 border-amber-300 rounded-xl hover:bg-amber-200">
              <Clock className="w-4 h-4"/>Actualizar
            </button>
          )}
        </div>
        <div className="flex gap-2 p-1 mt-5 border bg-white/60 rounded-xl border-amber-200 w-fit">
      ...
      {activeTab === 'access' && (
        <>
          ...
          <div className="flex flex-wrap gap-3 p-4 bg-white border-2 border-gray-200 rounded-xl">
            ...
            <span className="flex items-center gap-1.5 text-xs font-medium text-gray-600"><span className="w-3 h-3 bg-green-500 rounded-full"/>Confirmada</span>
            <span className="flex items-center gap-1.5 text-xs font-medium text-gray-600"><span className="w-3 h-3 bg-gray-400 rounded-full"/>Completada</span>
            ...
            <span className="flex items-center gap-1.5 text-xs font-medium text-red-500 ml-2"><span className="w-2 h-2 bg-red-500 rounded-full"/>Docs faltantes</span>
          ...
          <div className="flex items-start gap-3 p-4 border border-blue-200 bg-blue-50 rounded-xl">
            ...
            <p className="text-xs text-blue-700">Haz clic en un día del calendario para gestionar las visitas. Si hay varias aparece un selector con el estado de cada una. Los espacios bloqueados por Compras solo son de consulta.</p>
          </div>
        </>
      )}
      ...
      {accessDayModal && (
        <DayAppointmentsModal
          ...
          blocks={blocksByDate[accessDayModal] || []}
          mode="access"
          onSelect={(a) => {
      ...
      {recDayModal && (
        <DayAppointmentsModal
          ...
          blocks={blocksByDate[recDayModal] || []}
          mode="receptions"
