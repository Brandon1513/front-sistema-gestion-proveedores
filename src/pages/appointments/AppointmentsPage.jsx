...
const AppointmentModal = ({ appointment, onClose, preselectedDate }) => {
  ...
  const [form, setForm] = useState({
    provider_id:appointment?.provider?.id||'', appointment_date:appointment?.appointment_date||preselectedDate||'',
    appointment_time:appointment?.appointment_time?.slice(0,5)||'09:00', type:appointment?.type||'entrega',
  ...
  const set=(key,val)=>{setForm(f=>({...f,[key]:val}));if(key==='provider_id'){setConfirmed(false);setItemsState([]);}};
  const handleSubmit=(e)=>{
    ...
    mutation.mutate({...form,items:itemsState.map(i=>({product_service_id:i.product_service_id,quantity_expected:i.quantity_expected||null,unit_id:i.unit_id||null}))});
  };
      ...
      <div className="bg-white rounded-2xl shadow-2xl w-full max-w-2xl max-h-[92vh] overflow-y-auto">
        ...
        <form onSubmit={handleSubmit} className="p-6 space-y-5">
          ...
          <div className="grid grid-cols-2 gap-4">
            <div>
              <label className="block mb-1.5 text-sm font-semibold text-gray-700">Fecha *</label>
              <input type="date" value={form.appointment_date} onChange={e=>set('appointment_date',e.target.value)} min={new Date().toLocaleDateString('en-CA')} required className="w-full px-3 py-2.5 text-sm border-2 border-gray-200 rounded-xl focus:outline-none focus:border-primary-500"/>
            </div>
            <div>
              <label className="block mb-1.5 text-sm font-semibold text-gray-700">Hora *</label>
              <select value={form.appointment_time} onChange={e=>set('appointment_time',e.target.value)} required className="w-full px-3 py-2.5 text-sm border-2 border-gray-200 rounded-xl focus:outline-none focus:border-primary-500">
                {HOURS.map(h=><option key={h} value={h}>{h} hrs</option>)}
              </select>
            </div>
          </div>
          <div>
          ...
          <div>
            ...
            <textarea value={form.notes} onChange={e=>set('notes',e.target.value)} rows={2} maxLength={2000} className="w-full px-3 py-2.5 text-sm border-2 border-gray-200 rounded-xl focus:outline-none focus:border-primary-500 resize-none"/>
          </div>
          <div>
          ...
          {isEdit&&(
            <div>
              ...
              <select value={form.status} onChange={e=>set('status',e.target.value)} className="w-full px-3 py-2.5 text-sm border-2 border-gray-200 rounded-xl focus:outline-none focus:border-primary-500">
                <option value="scheduled">Agendada</option><option value="confirmed">Confirmada</option><option value="completed">Completada</option>
              </select>
            </div>
          )}
...
export const AppointmentsPage = () => {
  ...
  const stats=useMemo(()=>({total:appointments.length,scheduled:appointments.filter(a=>a.status==='scheduled').length,confirmed:appointments.filter(a=>a.status==='confirmed').length,missing_docs:appointments.filter(a=>a.has_missing_docs).length,no_show:appointments.filter(a=>a.status==='no_show').length}),[appointments]);
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-2 gap-3 sm:grid-cols-5">
        {[{label:'Este mes',value:stats.total,color:'blue'},{label:'Agendadas',value:stats.scheduled,color:'purple'},{label:'Confirmadas',value:stats.confirmed,color:'green'},{label:'No presentaron',value:stats.no_show,color:'orange'},{label:'Docs faltantes',value:stats.missing_docs,color:'red'}].map(s=>(
      ...
      <div className="flex items-center justify-between p-4 bg-white border-2 border-gray-200 rounded-xl">
        ...
        <div className="flex gap-1 p-1 bg-gray-100 rounded-xl">
          {[['month','Mes',LayoutGrid],['week','Semana',List]].map(([val,label,Icon])=>(
            <button key={val} onClick={()=>setView(val)} className={`flex items-center gap-1.5 px-3 py-1.5 text-sm font-semibold rounded-lg transition-all ${val===view?'bg-white text-primary-700 shadow-sm':'text-gray-500 hover:text-gray-700'}`}>
            ...
            {HOURS.map(hour=>(
              <div key={hour} className="grid grid-cols-8 border-b border-gray-100 min-h-[56px]">
                ...
                {weekDays.map((d,i)=>{
                  ...
                  return(
                    <div key={i} onClick={()=>openCreate(dateStr)} className={`p-1 border-r border-gray-100 cursor-pointer hover:bg-blue-50/40 ${isToday?'bg-primary-50/30':''}`}>
                      {hourAppts.map(a=>(
                        <div key={a.id} onClick={e=>{e.stopPropagation();setDetailAppointment(a);}}
